...
def save_schedule(subject: str, weekday: int, start_time: str, end_time: str, room: str, teacher_name: str = None):
    ...
    NOTE: 'schedules' is a VIEW over 'classes' (WHERE status='current').
    Writes are redirected to the underlying 'classes' table.
    """
    init_db()
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    # Resolve course_id from the first available course, or use a sentinel
    row_course = cursor.execute("SELECT id FROM courses LIMIT 1").fetchone()
    course_id = row_course[0] if row_course else "LEGACY"

    # Check duplicate in the underlying 'classes' table
    cursor.execute("""
        SELECT id FROM classes
        WHERE subject_name = ? AND weekday = ? AND start_time = ? AND room = ?
          AND status = 'current'
        ...
        cursor.execute("""
            INSERT INTO classes
                (course_id, academic_year, semester, term, subject_code,
                 subject_name, class_group, weekday, start_time, end_time,
                 room, professor_name, status)
            VALUES (?, 0, 0, '0', ?, ?, 'SCRAPER', ?, ?, ?, ?, ?, 'current')
        """, (course_id, subject[:20], subject, weekday, start_time, end_time, room, teacher_name or ""))
        conn.commit()
    conn.close()
...
def clear_schedules():
    """Clear legacy/seeded schedule slots from 'classes' (status='current', academic_year=0).

    NOTE: 'schedules' is a VIEW — deletes must target the underlying 'classes' table.
    Only removes rows inserted by save_schedule (academic_year=0) to avoid wiping real data.
    """
    init_db()
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM classes WHERE status = 'current' AND academic_year = 0")
    conn.commit()
    conn.close()
...
def sync_from_scraper():
    ...
    try:
        ...
        # NOTE: 'schedules' is a VIEW — all writes go to the underlying 'classes' table.
        day_of_week = raw.get("day_of_week")
        if day_of_week is not None:
            # Resolve course_id once for all inserts
            row_course = cursor.execute("SELECT id FROM courses LIMIT 1").fetchone()
            course_id = row_course[0] if row_course else "LEGACY"

            # Clear scraper-seeded slots for this weekday (academic_year=0 = scraper-seeded)
            cursor.execute(
                "DELETE FROM classes WHERE weekday = ? AND status = 'current' AND academic_year = 0",
                (day_of_week,)
            )
                    ...
                    cursor.execute("""
                        INSERT INTO classes
                            (course_id, academic_year, semester, term, subject_code,
                             ...
                             room, professor_name, status)
                        VALUES (?, 0, 0, '0', ?, ?, 'SCRAPER', ?, ?, ?, ?, ?, 'current')
                    """, (course_id, subject[:20], subject, day_of_week, start_time, end_time, room, teacher_name))
