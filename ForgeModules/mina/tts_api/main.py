...
# ---------------------------------------------------------------------------
# Módulo de Ingestão de PDFs Acadêmicos (MABI / UNESP Sorocaba)
# ---------------------------------------------------------------------------
from fastapi import UploadFile, File, Form
from src.utils.pdf_extractor_service import AcademicPdfPipeline

pdf_pipeline = AcademicPdfPipeline()

class PdfUrlRequest(BaseModel):
    url: str = Field(..., description="URL pública do PDF da UNESP")
    doc_type: str = Field("schedule", description="Tipo de documento: 'schedule' (horários) ou 'calendar' (calendário)")

@app.post("/academic/ingest-pdf", tags=["academic"])
async def ingest_pdf_file(
    file: UploadFile = File(..., description="Arquivo PDF de grade horária ou calendário"),
    doc_type: str = Form("schedule", description="'schedule' ou 'calendar'")
):
    """Recebe um arquivo PDF via upload, extrai o texto de forma ultraleve e ingere no academic.db."""
    try:
        content = await file.read()
        res = pdf_pipeline.process_pdf_full(content, doc_type=doc_type, source_name=file.filename or "upload.pdf")
        return res
    except Exception as exc:
        log.error("Erro na extração do PDF: %s", exc)
        raise HTTPException(500, f"Falha ao processar PDF: {exc}")


@app.post("/academic/ingest-pdf-url", tags=["academic"])
async def ingest_pdf_url(req: PdfUrlRequest):
    """Baixa o PDF diretamente da URL informada da UNESP e alimenta o banco academic.db da Mina."""
    try:
        filename = req.url.split("/")[-1] or "remote.pdf"
        res = pdf_pipeline.process_pdf_full(req.url, doc_type=req.doc_type, source_name=filename)
        return res
    except Exception as exc:
        log.error("Erro ao baixar/processar PDF via URL: %s", exc)
        raise HTTPException(500, f"Falha ao processar PDF remoto: {exc}")
