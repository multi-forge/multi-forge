...
DTB_SRC="$REPO_ROOT/ForgeCore/dtb/meson-g12a-btv-e10-enterprise.dtb"
if [[ ! -f "$DTB_SRC" ]]; then
    DTB_SRC="$REPO_ROOT/ForgeDB/devices/btv/e10/dtb/meson-g12a-btv-e10-enterprise.dtb"
fi
if [[ ! -f "$DTB_SRC" ]]; then
    err "DTB Enterprise não encontrado! Compilando do DTS..."
    dtc -I dts -O dtb "$REPO_ROOT/ForgeCore/dtb/meson-g12a-btv-e10-enterprise.dts" -o "$DTB_SRC"
fi
...
mkdir -p "$MOUNT_ROOT/opt/forgeos"/{bin,network,web,display,state,tests,branding}
cp -r "$REPO_ROOT/ForgeProvisioner/bin/."        "$MOUNT_ROOT/opt/forgeos/bin/"
cp -r "$REPO_ROOT/ForgeProvisioner/network/."    "$MOUNT_ROOT/opt/forgeos/network/"
cp -r "$REPO_ROOT/ForgeProvisioner/web/."        "$MOUNT_ROOT/opt/forgeos/web/"
cp -r "$REPO_ROOT/ForgeProvisioner/display/."    "$MOUNT_ROOT/opt/forgeos/display/"
cp -r "$REPO_ROOT/ForgeProvisioner/tests/."      "$MOUNT_ROOT/opt/forgeos/tests/"
cp -r "$REPO_ROOT/ForgeProvisioner/branding/."   "$MOUNT_ROOT/opt/forgeos/branding/"
...
chmod +x "$MOUNT_ROOT"/opt/forgeos/bin/*.sh \
         ...
         "$MOUNT_ROOT"/opt/forgeos/web/server.py \
         "$MOUNT_ROOT"/opt/forgeos/branding/*.sh \
         "$MOUNT_ROOT"/opt/forgeos/branding/forge-banner \
         "$MOUNT_ROOT"/opt/forgeos/install.sh 2>/dev/null || true
...
log "10. Executando teste virtual de boot (Kernel + U-Boot + Rootfs) via QEMU ARM64..."
if [ -f "$SCRIPT_DIR/qemu-verify-boot.sh" ]; then
    bash "$SCRIPT_DIR/qemu-verify-boot.sh" "$RAW_IMG" 45 || {
        err "FALHA NO TESTE VIRTUAL DE BOOT! Imagem corrompida não será empacotada."
        exit 1
    }
fi
